...
public class MemberService {
	...
	public String registProc(MemberDTO member) {
		...
		int typesCount = 0;
		if (pw.matches(".*[A-Z].*"))
			typesCount++; // (1) 영문 대문자 포함
		if (pw.matches(".*[a-z].*"))
			typesCount++; // (2) 영문 소문자 포함
		if (pw.matches(".*[0-9].*"))
			typesCount++; // (3) 숫자 포함
		if (pw.matches(".*[!@#$%^&*()_+\\-=\\[\\]{};':\",./<>?~`|\\\\].*"))
			typesCount++; // (4) 특수문자 포함
		 ...
		 * secretPass = encoder.encode(member.getPw()); member.setPw(secretPass);
		 */
		/*
		 ...
		 * 
		 * pw 컬럼의 크기를 암호문 크기와 같거나 크게 변경 ALTER TABLE db_quiz MODIFY pw varchar2(60);
		 * COMMIT;
		 */
	...
	public String loginProc(String id, String pw) {
		...
		if (check != null && check.getPw().equals(pw)) {
			...
			/*
			 * session.setAttribute("member", check); ${sessionScope.member.id}
			 * ${sessionScope.member.pw} ${sessionScope.member.userName}
			 */
			return "로그인 성공";
		}
	...
	/*
	 * // 1. 암호화 객체 생성 부분 주석 처리 // BCryptPasswordEncoder encoder = new
	 * BCryptPasswordEncoder();
	 * 
	 * // 2. 암호화 매칭(matches) 대신 일반 문자열(평문) 비교로 변경 //
	 * if(encoder.matches(member.getPw(), check.getPw())) { if(member.getPw() !=
	 * null && member.getPw().equals(check.getPw())) {
	 * 
